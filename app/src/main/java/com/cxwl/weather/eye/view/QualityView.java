...
public class QualityView extends View{
	...
	public void setData(List<EyeDto> dataList) {
		if (!dataList.isEmpty()) {
			...
			maxValue = tempList.get(0).quality;
			minValue = tempList.get(0).quality;
			for (int i = 0; i < tempList.size(); i++) {
				EyeDto dto = tempList.get(i);
				if (maxValue <= dto.quality) {
					maxValue = dto.quality;
				}
				if (minValue >= dto.quality) {
					minValue = dto.quality;
				}
			}

			maxValue = 500 + itemDivider/2;
			minValue = 0;
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (dto.quality > 0) {
				...
				textP.setTextSize(CommonUtil.dip2px(mContext, 10));
				float tempWidth = textP.measureText(dto.quality+"");
				canvas.drawText(dto.quality+"", dto.x-tempWidth/2, dto.y-CommonUtil.dip2px(mContext, 5f), textP);
			}
